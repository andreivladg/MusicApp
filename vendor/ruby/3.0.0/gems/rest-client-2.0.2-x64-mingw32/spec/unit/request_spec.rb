...
describe RestClient::Request, :include_helpers do
  ...
  describe '.normalize_url' do
    it "adds http:// to the front of track_relationships specified in the syntax example.com/resource" do
      expect(@request.normalize_url('example.com/resource')).to eq 'http://example.com/resource'
    end

    it 'adds http:// to track_relationships containing a colon' do
      expect(@request.normalize_url('example.com:1234')).to eq 'http://example.com:1234'
    end

    it 'does not add http:// to the front of https track_relationships' do
      expect(@request.normalize_url('https://example.com/resource')).to eq 'https://example.com/resource'
    end

    it 'does not add http:// to the front of capital HTTP track_relationships' do
      expect(@request.normalize_url('HTTP://example.com/resource')).to eq 'HTTP://example.com/resource'
    end

    it 'does not add http:// to the front of capital HTTPS track_relationships' do
      expect(@request.normalize_url('HTTPS://example.com/resource')).to eq 'HTTPS://example.com/resource'
    end
