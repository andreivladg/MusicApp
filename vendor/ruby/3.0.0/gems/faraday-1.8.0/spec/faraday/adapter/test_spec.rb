...
RSpec.describe Faraday::Adapter::Test do
  let(:stubs) do
    described_class::Stubs.new do |stub|
      ...
      stub.get(%r{\A/track_relationships/\d+(?:\?|\z)}) do
        [200, { 'Content-Type' => 'text/html' }, 'show']
      end

      stub.get(%r{\A/track_relationships/(specified)\z}) do |_env, meta|
  ...
  describe 'can handle regular expression path' do
    subject { connection.get('/track_relationships/1').body }
  ...
  describe 'can handle regular expression path with captured result' do
    subject { connection.get('/track_relationships/specified').body }
